...
export default function AllSessionsPage() {
    ...
    const filteredSessions = allSessions.filter((session) => {
        if (filters.course_id !== 'all' && String(session.course_id) !== filters.course_id) return false;
        if (filters.session_type !== 'all' && session.session_type !== filters.session_type) return false;
        return true;
    });
    ...
    const columns = [
        ...
        },
        {
            ...
            label: 'Location / Link',
            render: (_: unknown, row: { session_type: string; location?: string; meeting_url?: string }) => row.session_type === 'PHYSICAL' ? row.location : (row.meeting_url || 'System Generated'),
        },
    ];
    ...
    const extraFilters = (
        <div className="flex flex-col md:flex-row items-start md:items-center gap-3 w-full md:w-auto">
            ...
            <div className={`${showFilters ? 'flex' : 'hidden'} md:flex flex-col sm:grid sm:grid-cols-2 md:flex-row gap-2 w-full md:w-auto transition-all duration-200`}>
                ...
                <Select value={filters.session_type} onValueChange={(val) => setFilters({ ...filters, session_type: val })}>
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-2">
                            Manage sessions across all courses
                        </p>
                    </div>
                </div>
